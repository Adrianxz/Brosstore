...
switch ($_GET["op"]) {
    ...
    case 'listarp':
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
            "0"=>'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->CLIENTE_ID.')"><i class="fa fa-pencil"></i></button>'.' '.'<button class="btn btn-danger btn-xs" onclick="eliminar('.$reg->CLIENTE_ID.')"><i class="fa fa-trash"></i></button>',
            "1"=>$reg->CLIENTE_NOMBRE,
            "2"=>$reg->CLIENTE_APELLIDO,
            "3"=>$reg->CLIENTE_NUMIDENT,
            "4"=>$reg->CLIENTE_TELEFONO,
