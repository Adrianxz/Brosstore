...
$total_venta = $total_venta !== "" ? (float)$total_venta : 0;
...
switch ($_GET["op"]) {
    case 'guardaryeditar':
        if (empty($idventa)) {
            ...
            // Aquí iría el código de edición, si lo hubiera.
        }
        break;
    ...
    case 'listarDetalle':
        ...
        while ($reg = $rspta->fetch_object()) {
            ...
            <td>'.$reg->nombre.'</td>
            <td>'.$reg->cantidad.'</td>
            <td>'.number_format($reg->precio_venta, 2, ',', '.').' COP</td>
            <td>'.number_format($reg->descuento, 2, ',', '.').' COP</td>
            <td>'.number_format($reg->subtotal, 2, ',', '.').' COP</td></tr>';
        ...
        echo '<tfoot>
            ...
            <th></th>
            <th></th>
            <th><h4 id="total">COP '.number_format($total, 2, ',', '.').'</h4><input type="hidden" name="total_venta" id="total_venta"></th>
        </tfoot>';
        break;
    ...
    case 'listar':
        ...
        while ($reg = $rspta->fetch_object()) {
            ...
            $data[] = array(
                ...
                "6" => number_format($reg->total_venta, 2, ',', '.').' COP',
    ...
    case 'selectCliente':
        ...
        while ($reg = $rspta->fetch_object()) {
            echo '<option value='.$reg->CLIENTE_ID.'>'.$reg->CLIENTE_NOMBRE.'</option>';
        }
        break;
    ...
    case 'listarArticulos':
        ...
        while ($reg = $rspta->fetch_object()) {
            $data[] = array(
                ...
                "3" => $reg->codigo,
                "4" => $reg->stock,
                "5" => number_format($reg->precio_venta, 2, ',', '.').' COP',
